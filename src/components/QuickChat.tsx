...
const QuickChat = ({ id = "" }: { id?: string }) => {
  // react router dom
  let navigate = useNavigate();
  let location = useLocation();

  // check it's new chat default true
  const [IsItNewChat, setIsItNewChat] = useState<boolean>(true);
  ...
  useEffect(() => {
    if (chatSession.messages.length > 1) {
      // set title
      if (IsItNewChat) {
        // add here ai based title system i am doing it manually with user fisrt prompt
        setChatSession((prev) => {
          return { ...prev, title: chatSession.messages[1].message };
        });

        // set new false
        setIsItNewChat(false);
      }
